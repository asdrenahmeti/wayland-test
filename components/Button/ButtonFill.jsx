...
function Button({ style, link, type, text }) {
  ...
  const hoverType = type === 'dark' ? 'from-secondary to-tertiary' : type === 'light' ? 'from-secondary to-tertiary' : ''
    ...
    <Link href={link}>
      <a
        className={`${style} font-mulish font-bold text-sm uppercase px-5 py-2.5 border-transparent border-solid border-2 rounded-lg group overflow-hidden flex justify-center relative 
         ${styleType}`}
      >
        <span className={`absolute bottom-0 left-0 flex w-full h-0 mb-0 transition-all duration-200 ease-out transform translate-y-100 bg-gradient-to-br group-hover:h-full opacity-100 ${hoverType}`}></span>
